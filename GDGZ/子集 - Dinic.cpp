...
int main() {
	...
	_rep(i,1,n) a[i] = inl();
	int S = 0,T = kN - 1;
	_rep(i,1,n) if(a[i] & 1) odd.push_back(i);
				else even.push_back(i);
	for(auto &o : odd) for(auto &e : even)
		if(gcd(a[o],a[e]) == 1 && gcd(a[o] + 1,a[e] + 1) == 1)
			add_edge(o,e,1);
	for(auto &o : odd) add_edge(S,o,1);
	for(auto &e : even) add_edge(e,T,1);
	printf("%lld\n",n - max_flow(S,T));
	return 0;
}
