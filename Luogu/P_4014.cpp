...
int main() {
	int n = in();
	_rep(i,1,n) _rep(j,1,n) c[i][j] = in(), add_flow(i, j + n, 1, c[i][j]);
	_rep(i,1,n) add_flow(S, i, 1, 0), add_flow(i + n, T, 1, 0);
	printf("%d\n", minCostMaxFlow().second);
	_rep(i,1,n) g[i].clear(), g[n + i].clear();
	g[S].clear(), g[T].clear();
	_rep(i,1,n) _rep(j,1,n) add_flow(i, j + n, 1, c[i][j]);
	_rep(i,1,n) add_flow(S, i, 1, 0), add_flow(i + n, T, 1, 0);
