...
namespace MCMF{
	...
	pair<int,int> maxCostMaxFlow(void) {
		pair<int,int> tmp, ans = {0,0};
		while((tmp = spfa()) != pair<int,int> {-1, -1}) ans.first += tmp.first, ans.second += tmp.second;
		return ans;
	}
...
int a[kN], b[kN], c[150][150];
int main() {
	int n = in(), m = in();
	_rep(i,1,n) add_flow(S, i, a[i] = in(), 0);
	_rep(i,1,m) add_flow(i + n, T, b[i] = in(), 0);
	_rep(i,1,n) _rep(j,1,m) add_flow(i, j + n, INT_MAX, c[i][j] = in());
	printf("%d\n", minCostMaxFlow().second);
	g[S].clear(), g[T].clear();
	_rep(i,1,n + m) g[i].clear(); 
	_rep(i,1,n) add_flow(S, i, a[i], 0);
	_rep(i,1,m) add_flow(i + n, T, b[i], 0);
	_rep(i,1,n) _rep(j,1,m) add_flow(i, j + n, INT_MAX, c[i][j]);
	printf("%d\n", maxCostMaxFlow().second);
	return 0;
}